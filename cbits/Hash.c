...
{
    table->dir[segment] = pmMallocBytes(HSEGSIZE * sizeof(HashList *), 
					 "allocSegment");
}
...
allocHashList (HashTable *table)
{
    ...
    if ((hl = table->freeList) != NULL) {
        ...
        hl = pmMallocBytes(HCHUNK * sizeof(HashList), "allocHashList");
	cl = pmMallocBytes(sizeof (*cl), "allocHashList: chunkList");
        cl->chunk = hl;
...
allocHashTable_(HashFunction *hash, CompareFunction *compare)
{
    ...
    table = pmMallocBytes(sizeof(HashTable),"allocHashTable");
